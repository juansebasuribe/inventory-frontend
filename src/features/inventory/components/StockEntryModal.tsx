...
export const StockEntryModal: React.FC<StockEntryModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Crear movimiento de entrada usando el endpoint correcto
      await inventoryService.createMovement({
        movement_type: "entry",
        product_barcode: formData.product_code,
        quantity: formData.quantity,
        to_location_code: String(formData.warehouse_id), // warehouse_id es el código
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-md w-full shadow-xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">Entrada de Stock</h2>
          <button
            onClick={onClose}
            disabled={loading}
