// src/features/providers/components/ProviderEditModal.tsx

import React, { useState, useEffect } from 'react';
import { X, Building2, Save } from 'lucide-react';
import { providerService } from '../../../shared/services/providerService';
import type { Provider } from '../../../shared/types/product.types';
...
export const ProviderEditModal: React.FC<ProviderEditModalProps> = ({
  ...
  // Cargar datos del proveedor cuando cambia
  useEffect(() => {
    if (provider) {
      setFormData({
        name: provider.name || '',
        identification_type: provider.identification_type || 'nit',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedProvider = await providerService.updateProvider(provider.id, providerData);
      
      console.log('✅ Proveedor actualizado exitosamente:', updatedProvider);
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Actualiza la información del proveedor</p>
            </div>
          </div>
          <button
            onClick={handleClose}
